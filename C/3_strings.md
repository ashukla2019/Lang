...
- Difference between a character array and a string literal.
...
## Q2. Why do strings require `'\0'`?

Library functions need to know where the string ends.
...
Because `str` is a pointer, **not** an array.
...
## Q3. Does `strcpy()` allocate memory?
...
## Q4. Why is `strcpy()` dangerous?

It performs **no bounds checking**.

---

## Q5. Is `strncpy()` always safe?

No.

It may not append a null terminator.

---

## Q6. Why is repeatedly calling `strlen()` inside a loop inefficient?

Because each call scans the string again.
...
- How does `strncat()` work?
- How does `strcmp()` compare strings?
...
## Q1. Why can't we compare strings using `==`?

Because `==` compares addresses,

not string contents.

---

## Q2. What does `strcmp()` return?
...
## Q3. Does `strcmp()` return only `-1`, `0`, or `1`?
...
```
Difference Of First Mismatching Characters
```

Only the sign (negative, zero, positive) should be used.

---

## Q4. Why is `strcmp("A","a")` negative?
...
## Q5. Is `strncat()` completely safe?

No.

It still requires that the destination buffer has enough space.
...
because the exact positive value is implementation-dependent.
...
- How to search characters in a string
- How to search substrings
- How tokenization works
- How CSV parsing works
- Difference between `strchr()` and `strrchr()`
...
## Q1. Difference between `strchr()` and `strstr()`?
...
## Q2. Why does `strtok()` modify the string?
...
to create separate strings.

---

## Q3. Can `strtok()` be used on string literals?
...
## Q4. What does `strrchr()` return?

Pointer to the **last** occurrence.

---

## Q5. Does `strstr()` return the matched string?

No.

It returns a **pointer** to the first occurrence inside the original string.

---

## Q6. Time Complexity of `strchr()`?
...
Searching for an empty string returns `str` itself.
...
## Q2. Why is argv an array of pointers?
...
## Q4. Why is argv[0] the program name?

It is supplied by the operating system.
...
Need conversion to integers.

---

## Q6. What type is argv[1][0]?
...
which points to the program name.
...
is **not** an array inside the function.
...
This chapter contains the **most frequently asked string coding problems** in C interviews.

For each problem we'll cover:
...
- Why was `gets()` removed from C?
- Difference between `gets()` and `fgets()`
- Buffer overflow
- Safe string handling
- Undefined Behavior involving strings
- Common interview questions
...
This is valid because `p` points to the writable array `str`, not to a string literal.
...
This chapter focuses on **interview-style questions** rather than theory.

These are the kinds of questions frequently asked in:
...
- Cisco
- Samsung
- Embedded Linux Interviews

Many of these are **output prediction** questions.
...
✅ Know the difference between `sizeof()` and `strlen()` thoroughly.

✅ Understand the memory layout of string literals versus character arrays.

✅ Be comfortable with pointer arithmetic like `str + 2`, `*(str + i)`, and `2[str]`.

✅ Never attempt to modify a string literal.

✅ Be able to implement `strlen()`, `strcpy()`, `strcmp()`, and `strcat()` from scratch.

✅ Most embedded interviewers focus on your understanding of **memory, pointers, and undefined behavior**, not just memorizing library functions.
